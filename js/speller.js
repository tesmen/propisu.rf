...
function fullFill(number) {
	...
	//if (str == "") { result = str + "00"; }
	if (str.length >= 2) { return number; }
...
function numToText(num, objsex, format, obj){ //(numToText(input, 0, "text", money[0]))
	if (num > 999999999999999 || isNaN(num)) { return " (x_x) ";}
	...
	var line = "";
	if (num == 0 && format == "text") 	{ return "ноль " + obj[getCase(digits[0])]; }
	if (num == 0 && format == "number") { return "0 " + obj[getCase(digits[0])]; }
	if (format == "text"){
...
function example(number){
	str = String(number);
	dotPos = str.indexOf(".");
	result = str.substring(dotPos-2, dotPos+4);
...
function prepare(number){ // замена запятых на точки, удаление пробелов, оставляет два знака п зап без округл
	...
	if (dot >= 0){
			...
			switch(dif) {
				...
				default : result = result.substring(0, dot+3); break;
			}
		}else if( dot == -1){ result += ".00" }
